...
>
> **P3b — executed; every item became a recorded decision (not silent drift):**
>
> - **Mobile whites → role tokens.** On-dark text/icons use `colors.heroText`(`Soft`); anything on a teal/CTA fill follows the Spark ink-on-teal rule (`onCta`) — a white CTA label is a brand bug, fixed on sight. True theme-independent whites (splash over the fixed brand background) keep literals with justification comments.
> - **PopupHost re-themed.** The static slate palette was a bug — popup _chrome_ now derives from the instance theme like everything else (`makeStyles(theme)`); CMS-authored content already themed via PageScope.
> - **Blue/sea gradient triplets: TS canonical wins.** `tokens.css` now carries all six slots' `-base/-dark` triplets from `CLASS_ACCENTS`; web adopted them (its independently-derived slot 4/5 values retired). The design-tokens contract test asserts the full six-slot lockstep.
> - **Pre-rebrand leftovers retired at the defaults layer**: HeaderBuilder's indigo `#4f46e5` and the certificate-template `#101828`/`#52525b` field defaults become Spark values — new configs/templates only; persisted values untouched. Chart `MIX_COLORS` grey aligned to the AA `#74726c`. Web dashboard's SVG `#eeecf5` → surface-2's value (SVG attrs need literals; keep-in-step comments).
> - **Kept, deliberately**: the popups-editor checkerboard/canvas greys (functional editor affordances, not brand surfaces); web/admin `--bg` + `--border-strong` stay per-app (two chrome contexts, two creams — unification value ≈ 0, whole-canvas risk); mobile spacing **12/20 stay raw by rule** — the scale is 4/8/16/24/32 and intermediates are allowed for optical tuning; not every number needs a token.
> - **Verification limits, stated honestly**: interior member/admin pages require credentials (policy: never handled), so visual verification covers public surfaces + the value-level guarantees; the deltas are near-imperceptible token alignments or new-config-only default changes.
...
> **Status: shipped (P2).** `@lms/ui` now holds `useModalA11y` (single source; the per-app copies from #129 are re-export shims), the merged toast provider, the `createRequest()` fetch core behind both apps' `lib/api.ts`, and `useImageCropper` — the pan/zoom/crop mechanism all three croppers triplicated (each cropper file is now thin per-app markup over it; markup unification waits for P3's token merge). `ApiError` and the shared formatters live in `@lms/types` so mobile shares them too. **Shared `<Button>` (added post-P4):** `@lms/ui` now also exports a props-driven `<Button variant size block iconOnly>` for the two DOM apps — a Save vs a Cancel is one component with different props, not repeated `<button className="btn …">` markup. It emits canonical `btn--*` classes that both apps' `globals.css` style identically (admin already used the `btn--*` family; web gained grouped `btn--*` aliases beside its legacy single-dash `.btn-*`), so adopting it is a visual no-op; `type` passes through untouched so form-submit semantics are preserved. Mobile keeps its own `CtaButton` (React Native can't consume the DOM component). Still open from the charter: `AuthGate`/`AuthGuard` unification (app-coupled, deferred) and a `<Modal>` markup component (the contract is already enforced by the hook; extract when a new modal needs it).
