...
> **Status:** Draft for adoption · **Date:** 2026-08-14 · **Scope:** this monorepo (the licensing-dashboard control-plane repo should mirror these standards in a follow-up).
>
...
- ✅ `strict: true` in web, admin, control-plane, mobile, and all packages.
...
- ⚠️ **`apps/api` is the only non-strict workspace** (Nest starter config: no `strict`, `noImplicitAny: false`, `strictBindCallApply: false`) despite being the largest surface. → D7 ratchet.
- ⚠️ Compiler versions float: `^5.6.2` in api/db/bdd resolves to 5.9.3 while admin pins 5.5.4 exactly — four different compilers in the tree. Pin what CI uses.
