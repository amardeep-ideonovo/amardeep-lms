...
test("rewriteOrigin swaps every occurrence of the source origin", () => {
  assert.equal(
    rewriteOrigin(`${DEMO}/media/a.jpg`, DEMO, TARGET),
    `${TARGET}/media/a.jpg`,
  );
  assert.equal(
...
test("rewriteOrigin is a no-op when origins match or either is empty", () => {
  ...
  assert.equal(
    rewriteOrigin(`${DEMO}/media/a.jpg`, DEMO, ""),
    `${DEMO}/media/a.jpg`,
  );
});
...
test("normalizeRowForImport remaps author/uploader FKs to the target owner admin", () => {
  assert.equal(
    normalizeRowForImport("post", { id: "p1", authorId: "demoAdmin" }, "admin1")
      .authorId,
    "admin1",
  );
  assert.equal(
...
test("normalizeRowForImport remaps author to null when target has no owner admin", () => {
  assert.equal(
    normalizeRowForImport("post", { id: "p1", authorId: "demoAdmin" }, null)
      .authorId,
    null,
  );
});
