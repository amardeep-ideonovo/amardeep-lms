...
function makePrisma(overrides: Record<string, (args?: any) => any> = {}) {
  ...
  const fn = (name: string, fallback: (args?: any) => any) => {
    ...
    return (args?: any) => {
      ...
      return Promise.resolve((overrides[name] ?? fallback)(args));
    };
  };
  const prisma = {
    emailLog: {
      findUnique: fn("emailLog.findUnique", () => null),
      upsert: fn("emailLog.upsert", (a: any) => ({
        id: "upserted",
        ...a.create,
        ...a.update,
      })),
      create: fn("emailLog.create", (a: any) => ({ id: "created", ...a.data })),
      update: fn("emailLog.update", (a: any) => ({
        id: a.where.id,
        ...a.data,
      })),
    },
...
function makeSender(
  over: Partial<{ configured: boolean; send: (a: any) => any }> = {},
) {
  const seen: { send?: any } = {};
  return {
    seen,
...
async function withEnv(
  env: Partial<
    Record<"ENV_NAME" | "JWT_SECRET" | "SETTINGS_ENC_KEY", string | undefined>
  >,
  fn: () => Promise<void>,
) {
...
test("sendTemplate: with a signing secret, the unsubscribe link is rendered and sent", async () => {
  ...
  await withEnv(
    ...
    async () => {
      ...
      assert.equal(log.status, "SENT");
      assert.match(
        String((tplSeen.vars ?? {}).unsubscribeUrl),
        /\/unsubscribe\?token=/,
      );
      assert.match(
        String(senderSeen.send?.listUnsubscribe),
        /\/unsubscribe\?token=/,
      );
    },
  );
