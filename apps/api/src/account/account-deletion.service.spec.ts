...
test("contacts are TOMBSTONED (unsubscribed), never deleted", async () => {
  ...
  assert.ok(
    !h.calls.includes("contact.delete"),
    "contact must never be deleted",
  );
...
test("a member who never opted out is NOT newly suppressed", async () => {
  ...
  assert.ok(
    !h.calls.includes("contact.create"),
    "no tombstone for a non-opted-out member",
  );
});

test("concurrent double-delete (P2025) resolves idempotently, not a 500", async () => {
  const h = makeHarness({
    userDeleteThrows: new Prisma.PrismaClientKnownRequestError(
      "Record not found",
      {
        code: "P2025",
        clientVersion: "test",
