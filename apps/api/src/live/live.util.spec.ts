...
test("ZOOM: accepts zoom.us and its subdomains, https only", () => {
  ...
  assert.equal(
    providerHostAllowed("ZOOM", "https://us02web.zoom.us/j/123?pwd=x"),
    true,
  );
...
test("GOOGLE_MEET: accepts meet.google.com only", () => {
  assert.equal(
    providerHostAllowed("GOOGLE_MEET", "https://meet.google.com/abc-defg-hij"),
    true,
  );
  assert.equal(
    providerHostAllowed("GOOGLE_MEET", "https://zoom.us/j/1"),
    false,
  );
});
...
test("rejects look-alike and embedded-suffix hosts", () => {
  assert.equal(
    providerHostAllowed("ZOOM", "https://zoom.us.evil.com/j/1"),
    false,
  );
  assert.equal(providerHostAllowed("ZOOM", "https://notzoom.us/j/1"), false);
  assert.equal(
    providerHostAllowed("GOOGLE_MEET", "https://meet.google.com.evil.com/x"),
    false,
  );
...
test("parseZoomMeetingNumber extracts the numeric meeting id", () => {
  ...
  assert.equal(
    parseZoomMeetingNumber("https://zoom.us/wc/1234567890/join"),
    "1234567890",
  );
