...
// Branded route-error boundary — replaces the page content (inside the root
// layout's Nav + Footer) when a segment throws. We NEVER surface the raw error
// message or stack (that leaks internals); the optional `digest` is a
// support-correlatable hash Next already logs server-side.
export default function Error({
  ...
  useEffect(() => {
    // Next records this server-side; log client-side too so Sentry/console see it.
    console.error(error);
